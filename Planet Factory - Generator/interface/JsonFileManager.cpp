...
int	jsonFileManager::_parseSystem(SolarSystem * solarSystem)
{
    ...
    if(!json_is_string(type) || json_string_value(type) != "system")
    {
	...
	if(!json_is_string(materials))
	{
		std::cout << "error: materials is not an array" << std::endl;
		json_decref(root);
		return 1;
	}
	tmp = json_string_value(materials);
	solarSystem->materials(tmp);
