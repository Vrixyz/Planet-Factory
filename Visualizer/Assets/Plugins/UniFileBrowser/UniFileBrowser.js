...
function Awake () {
	...
	dateWidth = scrollViewStyle.CalcSize (GUIContent(System.DateTime.MinValue.ToString())).x + 8;
	...
	if (showVolumes && !limitToInitialFolder) {
		if (windowsSystem || linuxSystem) {
			...
			for (i = 0; i < drives.Length; i++) {
				dirList.Insert (i, new FileData(drives[i].Substring(0, drives[i].Length-1), System.DateTime.MinValue));
			}
			...
			try {
				...
				for (i = 0; i < numberOfVolumes; i++) {
					dirList.Insert (i, new FileData(dirInfo[i].Name, System.DateTime.MinValue));
				}
			}
			catch (err) {
				HandleError (err.Message);
